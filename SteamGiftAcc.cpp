...
MYERROR SteamGiftAcc::enterGA(const GiveAway &ga)
{
	...
	return MYERROR::NOT_ENOUGH_POINTS;
}
...
void SteamGiftAcc::parseGiveaway(const string &url, GArray *array) const
{
	...
	for (; it != end; ++it)
	{
		if (it->tagName() == "div")
		{
			...
			if (attr.first)
			{
				if (attr.second.find("is-faded") == string::npos && attr.second.find("giveaway__row-inner-wrap") != string::npos)
				{
					it += 6;
					it->parseAttributes();
